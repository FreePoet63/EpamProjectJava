...
@ThreadSafe
public class ShipDataBase<E> {
    @GuardedBy("this")
    private Queue<E> data = new LinkedList<E>();

    public synchronized void add(E e) {
        while (this.data.size() > 3) {
            System.out.println("Извините порт перегружен, "
        ...
        System.out.println(Thread.currentThread().getName()
                + " Доставил груз в порт, текущая загруженность порта "
                + this.data.size() + " из 3");
        notifyAll();
    }
    ...
    public synchronized E del() {
        while (this.data.size() == 0) {
            System.out.println("Извините, в порту груз отсутствует, "
                    + Thread.currentThread().getName()
        ...
        System.out.println(Thread.currentThread().getName()
                + " Произвёл отгрузку из порта, текущая заполненность порта "
                + this.data.size() + " из 3");
        notifyAll();
        return rsl;
    }
